...
import { Drink, NewPizza, Pizza, PizzeriaContext, User, AuthenticatedUser, MaybeAuthenticatedUser } from "../../types";
  ...
  const addPizza = async (newPizza: NewPizza) => {
    try {
      if (!authenticatedUser || authenticatedUser.username !== "admin") {
        throw new Error("You must be authenticated to add a pizza and be an admin");
      }
      const options = {
        method: "POST",
